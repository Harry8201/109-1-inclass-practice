...
Game5 <- list(
  Date = Date <- ("Oct.9"), 
  StartTime = StarTime <- ("21:00"), 
  TimeZone = TimeZone  <- ("ET"), 
  Away = Away <- ("Miami Heat"), 
  Home = Home <- ("L.A. Lakers")
)
...
Game5Time <- c(Game5$Date, Game5$StartTime, Game5$Timezone)
...
# G5TimeParsed
year <- ("2020")
Game5Timewithyear <- paste(Game5Time[[1]], year,Game5Time[[2]])

G5TimeParsed <- mdy_hm(Game5Timewithyear)
...
# TpeJason
TpeJason <- list(
  message = "今晚，我想再來點得分40+的Jimmy Bulter，佐大三元～", Timestamp = with_tz(G5TimeParsed, tz="Asia/Taipei")
)
...
ParisLouis <- Timestamp + minutes(15)
...
```{r ans21}
# sumChNumbers
chNumbers2 <- c(chNumbers)

sumChNumbers <- sum(as.numeric(factor(chNumbers2)))
...
ChNumbersWith01 <- c(chNumbersWith0)

sumChNumbersWith0 <- sum(as.numeric(factor(ChNumbersWith01)))
...
cat("在字串輸入時我們要用成對的或將文字包起來，R才會知道它是字串。"," \n", 
